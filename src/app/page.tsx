  ...
  return (
    <div className="font-sans text-[var(--ink)]">
      ...
      <header className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur-[14px]">
        <div className="mx-auto flex max-w-[1240px] items-center gap-3.5 px-5 py-3.5 sm:px-8">
          ...
          <div className="ml-auto flex items-center gap-3 sm:gap-5">
            ...
            <a href="/login" className="rounded-[10px] bg-gradient-to-br from-[#E4CE9A] to-[#C6A96A] px-[14px] py-2 sm:px-[18px] sm:py-2.5 text-[13px] sm:text-[14px] font-semibold text-[#14140F] transition-all press">
              Claim account →
            </a>
          </div>
        </div>
      ...
      <section className="relative flex min-h-[90vh] items-center overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-[900px] px-5 pb-[60px] pt-[72px] text-center sm:px-8">
          <div className="relative mb-[24px] sm:mb-[30px] inline-flex items-center justify-center animate-au-rise">
            <div className="absolute h-[280px] w-[280px] sm:h-[340px] sm:w-[340px] rounded-full bg-[radial-gradient(circle,rgba(198,169,106,0.30),transparent_62%)] animate-au-glow" />
            <Image src={logoSrc} alt="Aureus" width={236} height={236} className="relative w-[160px] sm:w-[236px] animate-au-float drop-shadow-2xl" />
          </div>
          <div className="animate-au-rise font-mono text-[11px] sm:text-[12px] uppercase tracking-[0.16em] text-[#93702C]" style={{ animationDelay: '0.12s' }}>
            Est · MMXXVI — private wealth, struck as one
          </div>
          <h1 className="animate-au-rise font-display text-[clamp(38px,9vw,88px)] font-medium leading-[0.98] tracking-[-0.02em] text-foreground" style={{ animationDelay: '0.2s' }}>
            Command your whole <em className="not-italic text-[#93702C]">fortune.</em>
          </h1>
          <p className="animate-au-rise mx-auto mt-5 sm:mt-6 max-w-[540px] text-[16px] sm:text-[18px] leading-[1.62] text-muted-foreground" style={{ animationDelay: '0.3s' }}>
          ...
          <div className="animate-au-rise flex flex-col sm:flex-row justify-center gap-3 sm:gap-3.5 px-4 sm:px-0" style={{ animationDelay: '0.4s' }}>
      ...
      {/* summary strip */}
      <div className="relative z-20 mx-auto -mt-6 max-w-[1240px] px-5 sm:px-8">
        <div className="rounded-[14px] border border-border bg-card px-[18px] py-[16px] sm:px-[22px] sm:py-[18px] font-mono text-[12px] sm:text-[13px] leading-[1.7] text-muted-foreground shadow-[0_8px_30px_rgba(80,70,45,0.06)]">
      ...
      <div className="mt-11 border-y border-border bg-secondary">
        <div className="mx-auto flex max-w-[1240px] flex-wrap items-center justify-center sm:justify-between gap-4 sm:gap-6 px-5 sm:px-8 py-5 font-mono text-[12px] uppercase tracking-[0.08em] text-faint">
      ...
      <section id="features" className="bg-background">
        <div className="mx-auto max-w-[1240px] px-5 sm:px-8 py-[60px] sm:py-[100px]">
          <div data-reveal className="mb-12 sm:mb-16 max-w-[640px]">
            <div className="font-mono text-[12px] uppercase tracking-[0.14em] text-[#93702C]">The console</div>
            <h2 className="mt-4 font-display text-[clamp(30px,5vw,52px)] font-medium leading-[1.06] tracking-[-0.01em] text-foreground">Six instruments. One standard.</h2>
          </div>
          <div data-reveal className="grid grid-cols-1 gap-px overflow-hidden rounded-[18px] border border-border bg-border md:grid-cols-2">
            {FEATURES.map((f, i) => (
              <div
                ...
                className="lift bg-card p-6 sm:p-9 transition-transform duration-200 hover:scale-[1.01]"
              >
      ...
      <section id="analytics" className="border-y border-border bg-secondary">
        <div data-reveal className="mx-auto grid max-w-[1240px] grid-cols-1 items-center gap-10 px-5 py-16 sm:px-8 sm:py-24 md:grid-cols-[0.9fr_1.1fr]">
          <div>
            <div className="font-mono text-[12px] uppercase tracking-[0.14em] text-[#93702C]">Know what you own</div>
            <h2 className="mt-4 font-display text-[clamp(26px,4vw,44px)] font-medium leading-[1.1] text-foreground">
              Two ETFs can hide the same ten stocks. Aureus shows you.
            </h2>
            <p className="mt-[18px] text-[15px] sm:text-[16.5px] leading-[1.65] text-muted-foreground">
            ...
            <div className="mt-8 flex gap-10">
              <div>
                <div className="font-display text-[34px] sm:text-[38px] text-[#93702C]">74.8%</div>
                <div className="mt-1 text-[13px] text-muted-foreground">in top 3 positions</div>
              </div>
              <div>
                <div className="font-display text-[34px] sm:text-[38px] text-[#93702C]">4.2</div>
                <div className="mt-1 text-[13px] text-muted-foreground">effective holdings</div>
              </div>
            </div>
          </div>
          <div className="rounded-[20px] border border-border bg-card p-5 sm:p-[30px] shadow-[0_12px_40px_rgba(80,70,45,0.06)]">
            <div className="mb-5 font-mono text-[11px] uppercase tracking-[0.14em] text-faint">Geographic · look-through</div>
            <div className="flex flex-col gap-3.5">
              {GEO.map((g) => (
                <div key={g.label}>
                  <div className="mb-1.5 flex justify-between text-[13px] sm:text-[14px] text-foreground">
                    <span>{g.label}</span>
      ...
      <section className="bg-background">
        <div data-reveal className="mx-auto max-w-[900px] px-5 py-16 sm:px-8 sm:py-24 text-center">
          <div className="font-display text-[40px] leading-[0.5] text-[#B99640]">&ldquo;</div>
          <p className="mt-5 font-display text-[clamp(20px,3.5vw,32px)] font-light leading-[1.4] text-foreground">
          ...
          <div className="mt-7 font-mono text-[12px] sm:text-[13px] uppercase tracking-[0.08em] text-faint">Rui — software engineer, Singapore</div>
        </div>
      </section>
      ...
      <section className="bg-background">
        <div className="mx-auto max-w-[1240px] px-5 pb-16 sm:px-8 sm:pb-24">
          <div data-reveal className="relative overflow-hidden rounded-[24px] border border-border bg-secondary p-8 sm:p-[76px] text-center">
            <Image
              src="/aureus/roman-1.png"
              alt=""
              fill
            ...
            <div className="relative">
              ...
              <h2 className="font-display text-[clamp(28px,5vw,52px)] font-medium leading-[1.05] text-foreground">Your capital deserves a mint.</h2>
              <p className="mx-auto mt-4 max-w-[460px] text-[16px] sm:text-[17px] text-muted-foreground">Join Aureus and see your whole financial life, struck as one.</p>
      ...
      <footer className="border-t border-border bg-secondary">
        <div className="mx-auto flex max-w-[1240px] flex-wrap items-center gap-4 px-5 py-8 sm:px-8 sm:py-11 text-faint">
          <div className="flex items-center gap-2.5">
          ...
          <div className="ml-4 flex flex-wrap gap-4 sm:ml-8 sm:gap-6 text-[13px]">
            <a href="#features" className="transition-colors hover:text-foreground">Product</a>
            <a href="#analytics" className="transition-colors hover:text-foreground">Analytics</a>
            <a href="#" className="transition-colors hover:text-foreground">Privacy</a>
            <a href="#" className="transition-colors hover:text-foreground">Security</a>
          </div>
          <span className="ml-auto font-mono text-[11px] sm:text-[12px] text-faint">© MMXXVI Aureus · Made in Singapore</span>
        </div>
      </footer>
    </div>
  )
