...
export default function DashboardPage() {
  const {
    ...
    accounts, totalCashBase, netWorthBase, fxRates,
  } = usePortfolio()
  const {
  ...
  return (
    <div className="-m-3 sm:-m-4 md:-m-6 lg:-m-8 border border-border rounded-sm overflow-hidden bg-card">
      ...
      {/* status bar */}
      <div className="flex flex-wrap items-center gap-x-5 gap-y-1 border-b border-border bg-background px-4 py-2 text-[11px]">
      ...
      {/* net worth hero — full width, no overlap */}
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            ...
            {loading
              ...
              : <div className="mt-1 text-3xl sm:text-4xl font-bold tabular-nums leading-none truncate">{formatCurrency(netWorthBase, base)}</div>}
          </div>
              ...
              <table className="w-full text-[11px]">
                <thead>
                  <tr className="bg-background text-muted-foreground">
                    ...
                    <th className="px-2 py-2 text-right font-medium text-[10px] tracking-[0.06em]">%</th>
                ...
                <tbody className="tabular-nums">
                  {topHoldings.map((h) => (
                    <tr key={h.id} className="border-t border-border">
                      <td className="px-3.5 py-2 font-bold">{h.ticker}<div className="text-[10px] font-normal text-muted-foreground truncate max-w-[140px]">{h.name ?? '—'}</div></td>
