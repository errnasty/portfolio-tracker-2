...
import {
  generateSuggestions, DEFAULT_PREFERENCES,
...
export default function ReportPage() {
  ...
  const suggestions = generateSuggestions(enriched, analytics, base, DEFAULT_PREFERENCES)
  ...
  return (
    <div className="space-y-6 print:space-y-3">
      ...
      <div className="report-content space-y-6 print:space-y-4">
        ...
        {/* Suggestions */}
        <Section title={`Suggestions (score ${Math.round(suggestions.score)} / 100 — ${suggestions.scoreLabel})`}>
          {suggestions.suggestions.length === 0 ? (
            <p className="text-sm text-muted-foreground">No actionable suggestions in your active focus areas.</p>
          ) : (
            <div className="space-y-3">
              {suggestions.suggestions.slice(0, 8).map((s) => (
                <div key={s.id} className={`rounded border p-3 text-sm ${
                  s.severity === 'critical' ? 'border-red-500/40' :
                  s.severity === 'warning' ? 'border-amber-500/40' :
                  s.severity === 'positive' ? 'border-emerald-500/40' :
                  'border-sky-500/40'
                }`}>
                  <div className="font-semibold">[{s.severity.toUpperCase()}] {s.title}</div>
                  <div className="text-xs text-muted-foreground mb-1">{s.summary}</div>
                  {s.explanation[0] && <p className="text-xs">{s.explanation[0]}</p>}
                </div>
              ))}
            </div>
          )}
